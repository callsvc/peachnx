...
#pragma pack(pop)
    ...
    class NcaMount {
    public:
        explicit NcaMount(const SysFs::VirtFilePtr& nca, const FsEntry& fsInfo, u32 index);
        SysFs::VirtFilePtr MountEncryptedFile(const std::array<u8, 32>& value, NCA& nca);
    ...
    private:
        FsHeader header;
        const SysFs::VirtFilePtr& parent;
